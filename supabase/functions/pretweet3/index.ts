...
/**
 * Fetches the latest tweetgenerationflow record with pretweet2 data
 */
async function fetchLatestTweetGenerationRecord(supabase) {
  ...
  while (retryCount < MAX_RETRIES) {
    try {
      ...
      if (error) {
        log('error', `Error fetching latest record (attempt ${retryCount + 1})`, error);
        throw error;
      }
      
      if (!data || data.length === 0) {
        log('warn', `No records with pretweet2 found (attempt ${retryCount + 1})`);
        throw new Error("No records with pretweet2 found");
      }
      
      if (!data[0].pretweet2) {
        log('warn', `Latest record has no pretweet2 data (attempt ${retryCount + 1})`);
        throw new Error("Latest record has no pretweet2 data");
      }
      
      if (!data[0].geminiobservation) {
        log('warn', "Latest record has no geminiobservation data, but will continue");
      }
      
      log('info', `Using tweetgenerationflow record: ${data[0].id}`);
      return data[0];
      ...
      if (retryCount < MAX_RETRIES) {
        ...
        throw new Error(`Failed to fetch latest record after ${MAX_RETRIES} attempts: ${error.message}`);
      }
    }
  }
}
...
/**
 * Calls the Gemini API and handles retries
 */
...
/**
 * Updates the tweetgenerationflow record with pretweet3 content
 */
async function savePretweet3Result(supabase, recordId, pretweet3Content) {
  let retryCount = 0;
  ...
  while (retryCount < MAX_RETRIES) {
    try {
      const { error } = await supabase
        .from('tweetgenerationflow')
        .update({ pretweet3: pretweet3Content })
        .eq('id', recordId);
      
      if (error) {
        log('error', `Error updating record with pretweet3 (attempt ${retryCount + 1})`, error);
        throw error;
      }
      
      log('info', "pretweet3 content saved successfully");
      return;
      ...
      if (retryCount < MAX_RETRIES) {
        ...
        throw new Error(`Failed to save pretweet3 content after ${MAX_RETRIES} attempts: ${error.message}`);
      }
    }
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch the latest record with pretweet2 data
    const record = await fetchLatestTweetGenerationRecord(supabase);
    
    log('info', "Retrieved pretweet2 and geminiobservation content, preparing Gemini API request...");
    
    // Create system prompt for categorization
    const systemPrompt = createSystemPrompt();
    
    // Prepare user prompt with the record data
    const userPrompt = `Please analyze the following angles and geminiobservation, and classify them according to the categorization system:
...
${record.geminiobservation || "No geminiobservation available."}
    ...
    log('info', "Calling Gemini API for categorization...");
    const analysisResult = await callGemini(env.geminiApiKey, systemPrompt, userPrompt);
    
    log('info', "Categorization completed. Saving results to database...");
    log('debug', "Analysis result length:", analysisResult.length);
    
    // Save the analysis result to pretweet3 column
    await savePretweet3Result(supabase, record.id, analysisResult);
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        recordId: record.id,
        message: "Tweet categorization completed and saved successfully.",
