...
async function fetchTweetGenerationRecord(supabase, recordId) {
  ...
  if (!record.pretweet1) {
    throw new Error(`Record ${recordId} has no pretweet1 data`);
  }
...
/**
 * Creates the system prompt for Gemini API
 ...
 * @param {string} userPrompt - User prompt with content angles
 * @returns {Promise<string>} Selected content angles
  ...
  while (retryCount < MAX_RETRIES) {
    try {
      ...
      if (!response.ok) {
        ...
        log('error', `Gemini API error (${response.status})`, errorData);
...
/**
 * Updates a tweetgenerationflow record with pretweet2 result
 * @param {SupabaseClient} supabase - Supabase client
 * @param {string} recordId - Record ID to update
 * @param {string} pretweet2Content - Content to save to pretweet2
 * @throws {Error} If update fails
 */
async function savePretweet2Result(supabase, recordId, pretweet2Content) {
  let retryCount = 0;
  ...
  while (retryCount < MAX_RETRIES) {
    const { error } = await supabase
      .from('tweetgenerationflow')
      .update({
        pretweet2: pretweet2Content
      })
      .eq('id', recordId);
      
    if (error) {
      updateError = error;
      log('error', `Error updating record with pretweet2 (attempt ${retryCount + 1}/${MAX_RETRIES})`, error);
      
      // Exponential backoff for retries
      if (retryCount < MAX_RETRIES - 1) {
        const delay = RETRY_DELAY_MS * Math.pow(2, retryCount);
        log('info', `Retrying database update in ${delay}ms...`);
        await new Promise(resolve => setTimeout(resolve, delay));
      }
      
      retryCount++;
    } else {
      log('info', "Pretweet2 content saved to database successfully");
      return;
    }
  }
  
  // If we've reached here, all retries failed
  throw new Error(`Failed to save pretweet2 content to database after ${MAX_RETRIES} attempts: ${updateError?.message}`);
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch the pretweet1 data from the record
    const record = await fetchTweetGenerationRecord(supabase, recordId);
    
    log('info', "Retrieved pretweet1 data, preparing Gemini API request...");
    
    // Create system prompt and user prompt
    const systemPrompt = createSystemPrompt();
    const userPrompt = `Please evaluate the following content angles and select the two most promising ones based on the evaluation framework:

${record.pretweet1}

Remember to only include the two selected angles in your response, exactly as they appear in the original input, with no additional text or modifications.`;
    
    // Call Gemini API
    log('info', "Calling Gemini API to select content angles...");
    const selectedAngles = await callGeminiAPI(env.geminiApiKey, systemPrompt, userPrompt);
    
    log('info', "Content angles selected. Saving results to database...");
    log('debug', "Selected angles length:", selectedAngles.length);
    log('debug', "Selected angles (first 200 chars):", selectedAngles.substring(0, 200));
    
    // Save the selected angles to the database
    await savePretweet2Result(supabase, recordId, selectedAngles);
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        recordId: recordId,
        message: "Content angles selected and saved successfully",
        contentLength: selectedAngles.length
      }),
